...
python do_cyclonedx_package_collect() {
    ...
    # append any cve status within recipe to pn_list cve_status
    cves = {}
    for cve in (d.getVarFlags("CVE_STATUS") or {}):
        append_to_vex(d, cve, cves)
...
def append_to_vex(d, cve, cves):
    ...
    cves[cve] = { "detail_string": detail_string, "vex_state": vex_state }
    return
    ...
    for pkg in recipes:
        ...
        for pn_pkg in pn_list["pkgs"]:
            ...
            for sbom_pkg in sbom["components"]:
                ...
                if pn_list["cves"]:
                    vex["vulnerabilities"].append(pn_pkg)
